...
export function generateStaticParams() {
  const units = calculatorsByCategory("featured-units");
  return units.map((c) => ({ slug: c.slug }));
}
...
function adjacent(slug: string) {
  const units = calculatorsByCategory("featured-units");
  const i = units.findIndex((c) => c.slug === slug);
  return {
    prev: i > 0 ? units[i - 1] : units[units.length - 1],
    next: i < units.length - 1 ? units[i + 1] : units[0],
  };
}
...
export default async function FeaturedUnitPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const calc = getCalculator(slug);
  if (!calc || calc.category !== "featured-units") notFound();
      ...
      <JsonLd
        data={[
          ...
          breadcrumbLd([
            ...
            { name: calc.title, url: `/featured-units/${calc.slug}` },
          ]),
        ]}
      />
      ...
      <section className="border-b bg-surface">
        <div className="container-page pt-5 pb-8">
          <Breadcrumbs
            items={[
              { label: "Home", href: "/" },
              { label: "Featured Units", href: "/featured-units" },
              { label: calc.title.replace(" Calculator", "") },
            ]}
          />
          <div className="mt-5 max-w-3xl">
            {calc.subcategory && (
              <Link href={`/featured-units#${calc.subcategory}`} className="chip mb-3">
      ...
      <div className="container-page section grid gap-10 lg:grid-cols-[1fr_220px]">
        <div className="min-w-0 flex flex-col gap-14 order-2 lg:order-1">
          ...
          <nav className="grid gap-3 sm:grid-cols-2 pt-2" aria-label="Adjacent calculators">
            <Link href={`/featured-units/${prev.slug}`} className="card card-pad hover:border-brand transition-colors group">
          ...
          <div className="sticky top-20">
            ...
            <div className="mt-5 pt-5 border-t">
              <Link href="/featured-units" className="link text-sm inline-flex items-center gap-1">
                <ArrowLeft size={14} /> All unit conversions
              </Link>
            </div>
          </div>
        </aside>
