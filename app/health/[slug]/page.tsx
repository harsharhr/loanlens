...
export function generateStaticParams() {
  const tools = calculatorsByCategory("health");
  return tools.map((c) => ({ slug: c.slug }));
}
...
export default async function HealthToolPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const calc = getCalculator(slug);
      ...
      <JsonLd
        data={[
          ...
          breadcrumbLd([
            ...
            { name: calc.title, url: `/health/${calc.slug}` },
          ]),
        ]}
      />
      ...
      <section className="border-b bg-surface">
        <div className="container-page pt-5 pb-8">
          ...
          <div className="mt-5 max-w-3xl">
            {calc.subcategory && (
              <Link href={`/health#${calc.subcategory}`} className="chip mb-3">
                {calc.subcategory.replace("-", " ")}
              </Link>
            )}
            ...
            {/* Highly visible medical disclaimer block at the top of the content */}
            <div className="mt-5 flex gap-3 rounded-lg border border-warning/30 bg-warning/5 px-4 py-3">
              <AlertTriangle size={18} className="text-warning shrink-0 mt-0.5" />
              <p className="text-sm text-ink-secondary leading-relaxed font-medium">
                {calc.disclaimer ?? "This calculator provides estimates for educational purposes only. It is not a substitute for professional medical advice, diagnosis, or treatment. Always seek the advice of your physician or other qualified health provider."}
              </p>
            </div>
      ...
      <div className="container-page section grid gap-10 lg:grid-cols-[1fr_220px]">
        <div className="min-w-0 flex flex-col gap-14 order-2 lg:order-1">
          ...
          <nav className="grid gap-3 sm:grid-cols-2 pt-2" aria-label="Adjacent calculators">
            <Link href={`/health/${prev.slug}`} className="card card-pad hover:border-brand transition-colors group">
          ...
          <div className="sticky top-20">
            ...
            <div className="mt-5 pt-5 border-t">
              <Link href="/health" className="link text-sm inline-flex items-center gap-1">
                <ArrowLeft size={14} /> All health tools
              </Link>
            </div>
          </div>
        </aside>
