...
import { getCalculator, calculatorsByCategory, relatedCalculators } from "@/lib/calculators/registry";
import { JsonLd, calculatorLd, faqLd, breadcrumbLd } from "@/lib/seo";
import { SITE } from "@/lib/site";

export function generateStaticParams() {
  return calculatorsByCategory("everyday").map((c) => ({ slug: c.slug }));
}
...
export async function generateMetadata({
  ...
  if (!calc || calc.category !== "everyday") return {};
...
function adjacent(slug: string) {
  ...
  const i = tools.findIndex((c) => c.slug === slug);
  return {
    prev: i > 0 ? tools[i - 1] : tools[tools.length - 1],
    next: i < tools.length - 1 ? tools[i + 1] : tools[0],
  };
}
...
export default async function EverydayToolPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const calc = getCalculator(slug);
  if (!calc || calc.category !== "everyday") notFound();
      ...
      <JsonLd
        data={[
          ...
          breadcrumbLd([
            ...
            { name: calc.title, url: `/tools/${calc.slug}` },
          ]),
        ]}
      />
      ...
      <section className="border-b bg-surface">
        <div className="container-page pt-5 pb-8">
          ...
          <div className="mt-5 max-w-3xl">
            <Link href="/tools" className="chip mb-3">
              Everyday Tools
            </Link>
              ...
              <div className="mt-5 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-muted border-t pt-4">
                ...
                {calc.lastUpdated && (
                  <div className="flex items-center gap-1.5">
                    ...
                    <time dateTime={calc.lastUpdated}>
                      {new Date(calc.lastUpdated).toLocaleDateString("en-IN", { year: "numeric", month: "long", day: "numeric" })}
                    </time>
                  </div>
                )}
              </div>
      ...
      <div className="container-page section grid gap-10 lg:grid-cols-[1fr_220px]">
        <div className="min-w-0 flex flex-col gap-14 order-2 lg:order-1">
          ...
          <section id="how" className="scroll-mt-24 prose-fin max-w-none">
            <h2>How the {calc.title.replace(" Calculator", "").toLowerCase()} works</h2>
          ...
          <div className="flex gap-3 rounded-lg border border-line bg-brand-tint/40 px-4 py-4">
            ...
            <p className="text-sm text-muted leading-relaxed">
              {calc.disclaimer ??
                "This calculator is for informational purposes only. For legally binding date or eligibility calculations, verify against the exact rule in the relevant notification, contract or statute."}
            </p>
          </div>
          ...
          <nav className="grid gap-3 sm:grid-cols-2 pt-2" aria-label="Adjacent calculators">
            <Link href={`/tools/${prev.slug}`} className="card card-pad hover:border-brand transition-colors group">
        ...
        <aside className="order-1 lg:order-2 hidden lg:block">
          <div className="sticky top-20">
            ...
            <div className="mt-5 pt-5 border-t">
              <Link href="/tools" className="link text-sm inline-flex items-center gap-1">
                <ArrowLeft size={14} /> All everyday tools
              </Link>
            </div>
          </div>
        </aside>
